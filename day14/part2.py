...
def spin(prev_r: frozenset[complex]):
    eastl = [set[complex]() for _ in range(south_end)]
    for old in prev_r:
        eastl[int(old.real)].add(old)
    northl = [set[complex]() for _ in range(east_end + 1)]
    for olds in eastl:
        for old in olds:
            while (
                old.real > 0
                and old - 1 not in s_rocks
                and old - 1 not in northl[int(old.imag)]
            ):
                old -= 1
    ...
    for olds in northl:
        for old in olds:
            while (
                old.imag > 0
                and old - 1j not in s_rocks
                and old - 1j not in eastl[int(old.real)]
            ):
                old -= 1j
            eastl[int(old.real)].add(old)
    for x in northl:
    ...
    for olds in northl[::-1]:
        for old in olds:
            while (
                old.imag < east_end and old + 1j not in s_rocks and old + 1j not in east
            ):
                old += 1j
            east.add(old)
